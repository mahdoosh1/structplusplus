...
def parseFile(data: bytes, offset: int = 0) -> tuple[dict, int]:
    ctx = {}
    ctx['full'] = len(data)//40-1
    ctx['actions'], offset = type_array(data, offset, parseActions, int(ctx['full']), ({},))
    return ctx, offset

def parseActions(data: bytes, offset: int, extras: dict) -> tuple[dict, int]:
    ctx = {}
